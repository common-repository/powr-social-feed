<?php
    ...
    Plugin URI: https://www.powr.io/plugins/social-feed
    Description: Get more Likes and Followers with automatic social streaming  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Social_Feed extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Social Feed.
        </p>
        <p>
    ...
    function powr_social_feed_menu() {
      add_menu_page( 'POWr Social Feed', 'POWr Social Feed', 'manage_options', 'powr-social-feed-settings', 'powr_social_feed_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_social_feed_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/social-feed';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/social-feed';
      }
    ...
    register_activation_hook(__FILE__, 'powr_social_feed_plugin_activate');
      add_action('admin_init', 'powr_social_feed_plugin_redirect');

      function powr_social_feed_plugin_activate() {
      add_option('powr_social_feed_plugin_do_activation_redirect', true);
      }
      ...
      function powr_social_feed_plugin_redirect() {
      if (get_option('powr_social_feed_plugin_do_activation_redirect', false)) {
          delete_option('powr_social_feed_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-social-feed-settings&' );
          }
       }
      }
